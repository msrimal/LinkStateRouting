...
void * receive_info (void *arg)
{
    ...
    while (1) {
        ...
        int id = ntohl(packet[0]);
        int neighbor_id = ntohl(packet[1]);
        int new_cost = ntohl(packet[2]);

        // Update cost table
        pthread_mutex_lock(&lock);
        costs[id][neighbor_id] = new_cost;
        costs[neighbor_id][id] = new_cost;
        pthread_mutex_unlock(&lock);

        printf("Received new cost from %d to %d: %d\n", id, neighbor_id, new_cost);
        print_costs();
    }
...
void * run_link_state (void *arg)
{
	...
	while (1)
	{
		...
		{
			// find closest node
			min = INFINITE; //reset min at the beginning
			spot = -1;
            for (j = 0; j < N; j++) {
